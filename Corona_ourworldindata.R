...
setwd("~/Documents/R/CoronaVirus/Ourworldindata")
...
df1 = df[df$location %in% names(which(deaths_max>10000)),]
df2 = df[df$location %in% names(which(deaths_max<=10000 & deaths_max>3000)),]
df3 = df[df$location %in% names(which(deaths_max<=3000 & deaths_max>1000)),]
df4 = df[df$location %in% names(which(deaths_max<=1000)),] 
...
p1 = ggplot() +
  labs(title = paste("Covid-19 deaths by countries (",df$date[length(df$date)],")",sep = ""), subtitle = "source : https://covid.ourworldindata.org") +
  xlab("") + ylab("deaths") + xlim(as.Date(c("2020-01-01","2020-05-10"))) + 
  scale_y_continuous(trans = "log10",limits = c(1,100000),labels = scales::comma) +
  theme_minimal() + theme(legend.position = "none") +
  geom_line(data = df4,aes(x = date, y = total_deaths, group = location),colour = pal[4]) +
  geom_line(data = df3,aes(x = date, y = total_deaths, group = location),colour = pal[3]) +
  geom_line(data = df2,aes(x = date, y = total_deaths, group = location),colour = pal[2]) +
  ...
  geom_dl(data = df3,aes(x = date, y = total_deaths,label = location),colour = pal[3], method = list(dl.trans(x = x + .2),"last.bumpup"))
print(p1)
ggsave(plot=p1, filename=paste("Countries_log_",df1$date[dim(df1)[1]],".png",sep = ""), height=10, width=10)
