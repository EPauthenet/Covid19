...
n0 = 150000 ; n1 = 10000 ; n2 = 3000 ; n3 = 1000 ; n4 = 400
deaths_max = by(df$total_deaths,df$location,max)
df1 = df[df$location %in% names(which(deaths_max<=n0 & deaths_max>n1)),]
...
df4b = df[df$location %in% c("Denmark","Finland","Norway","Poland"),] #Only a few greys

#Define different zooms
df_rect=data.frame(x1 = as.Date(c("2020-03-25","2020-03-31"))
  ,x2 = c(as.Date(c("2020-06-15","2020-06-15")))
  ,y1 = c(0,0)
  ,y2 = c(n1,n2))

pal = c("firebrick","orange","steelblue","grey")
p1 = ggplot() +
  labs(title = paste("Covid-19 deaths by countries (",max(df$date),")",sep = ""), subtitle = "source : https://covid.ourworldindata.org / Graphic : E. Pauthenet") +
  xlab("") + ylab("deaths") + xlim(as.Date(c("2020-01-25",df_rect$x2[2]))) + 
  ...
  annotate("text", x = as.Date("2020-02-25"), y = 60000, label = max(df$total_deaths),col = 'grey',size = 10)
#
p2 = ggplot() +
...
p3 = ggplot() +
  ...
  geom_dl(data = df4b,aes(x = date, y = total_deaths,label = location),colour = "grey", method = list(dl.trans(x = x + .2),"last.qp")) +
  geom_rect(aes(xmin=x1, xmax=x2,ymin=y1, ymax=y2),alpha = 0, color=pal[c(2,3)],data = df_rect,lty = 3)
#
